...
$("#allPakaiTables").DataTable({
	...
	deferRender: true,
	ajax: {
		url: "http://localhost/he-bengkel/oper/getPakaiAll",
		type: "POST",
		dataType: "json",
	},
	columns: [
		...
		},
		{
			...
			render: function (data, type, row) {
				if (row.jml_pakai != 0) {
					return (
						'<a href="javascript:void(0);" class="btn btn-sm border border-light btn-danger text-white btn-oper" data-id="' +
						row.id_detail_pakai +
						'">Oper</a>'
					);
				} else {
					return '<button type="button" class="btn btn-sm border border-light btn-dark text-white" disabled>Oper</button>';
				}
			},
		},
	],
...
$("#allPakaiTables").on("click", ".btn-oper", function () {
	...
	$.ajax({
		url: "http://localhost/he-bengkel/oper/getDetailPakaiForOper",
		type: "POST",
		data: {
			id: id,
		},
